...
//! MediaPipe Selfie Segmentation via tract.
//!
//! MobileNetV3-based person segmentation (106K params, ~259KB gzipped).
//! Input: NCHW RGB float32 [0, 1], 256x256.
//! Output: single-channel alpha matte [1, 1, 256, 256] in [0, 1].
//!
//! This is a person-only segmentation model (not general salient object
//! detection). It's useful as a fast saliency proxy since most photos
//! that need smart cropping contain people.
//!
//! The ONNX model is simplified (onnxsim) with Resize ops converted to
//! scales-based for tract compatibility.

use tract_onnx::prelude::*;
use zensally::{ImageRef, PixelFormat, SaliencyDetector, SaliencyMap};

/// Embedded gzip-compressed MediaPipe Selfie Segmentation ONNX model.
const MODEL_GZ: &[u8] = include_bytes!("../models/selfie_seg.onnx.gz");
...
impl SelfieSeg {
    /// Create a new selfie segmentation detector.
    ...
    fn preprocess(&mut self, pixels: &[u8], src_w: u32, src_h: u32, format: PixelFormat) {
        ...
        for dst_y in 0..INPUT_SIZE {
            ...
            for dst_x in 0..INPUT_SIZE {
                ...
                // NCHW RGB, scale to [0,1] only (no normalization)
...
impl SaliencyDetector for SelfieSeg {
