...
use zensally_tract::UltraFaceDetector;

#[test]
fn detect_portrait_ultraface() {
    ...
    let mut detector = UltraFaceDetector::new().expect("failed to create UltraFace detector");
    let faces = detector.detect(&ImageRef::new(pixels, w, h, PixelFormat::Rgb).unwrap());

    eprintln!("UltraFace detected {} faces:", faces.len());
